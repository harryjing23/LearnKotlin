...
// （接口中的所有成员都是open的，也不需要显式使用open）
abstract class Animated {
    abstract fun animate()
}

// Kotlin可见性修饰符有public、internal、protected、private。没有Java默认的package-private
// public是默认的，internal是只在模块内部可见，private只在同一文件内可见，protected只在子类可见
...
// ### Kotlin的internal修饰符在Java中变成public，但internal成员名会被破坏，从而避免意外使用
// ### Kotlin的外部类不能访问其内部类的private成员，而Java可以

// Java中内部类会持有外部类的实例，所以可访问外部类的成员（static内部类不会）
// ### Kotlin的内部类与Java的static内部类类似，要持有外部类的引用需要用inner修饰符（相当于Java的内部类）

// 引用外部类实例的语法this@外部类类名。Java中用外部类类名.this，eg. User.this
class Outer {
    inner class Inner {
...
// 密封类。用sealed修饰，不可实例化（构造函数必须是private，默认有一个空的无参构造函数），在Java中是个抽象类
// 子类必须嵌套在父类中，且子类不能是data类。Kotlin 1.1只要求子类与密封类在同一文件中声明
// when结构用作表达式（有返回值）时，必须要有else分支。sealed类作为条件时则不需要else，但条件必须遍历所有子类
sealed class Expr {
